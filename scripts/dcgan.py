...
discrimator = models.Model(discriminator_input, discriminator_output)
...
x = layers.BatchNormalization(momentum=0.9)(x)
x = layers.LeakyReLU(0.2)(x)
generator_output = layers.Conv2DTranspose(1,
                                          kernel_size=4,
                                          strides=2,
                                          padding='same',
                                          use_bias=False,
...
class DCGAN(models.Model):
    ...
    def __init__(self, discrimator, generator, latent_dim):
    ...
    def train_step(self, real_images):
        ...
        with tf.GradientTape() as gen_tape, tf.GradientTape() as disc_tape:
            generated_images = self.generator(random_latent_vector,
                                              training=True)
            real_predictions = self.discrimator(real_images, training=True)
            fake_predictions = self.discrimator(generated_images,
                                                training=True)
        ...
        gradients_of_discriminator = disc_tape.gradient(
            d_loss, self.discrimator.trainable_variables)
        gradients_of_generator = gen_tape.gradient(
            g_loss, self.generator.trainable_variables)

        self.d_optimizer.apply_gradients(
            zip(gradients_of_discriminator, discrimator.trainable_variables))
...
dcgan = DCGAN(discrimator=discrimator, generator=generator, latent_dim=100)
dcgan.compile(
